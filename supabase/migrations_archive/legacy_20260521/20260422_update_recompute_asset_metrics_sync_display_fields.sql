...
comment on function public.recompute_asset_metrics(uuid) is
  'Recomputes assets numeric metrics and display fields(value/change) from non-hidden, non-deleted holdings. USD holdings are converted with latest USD/KRW exchange rate.';

-- Backfill using the updated function definition
